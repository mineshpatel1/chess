...
class TestPieces(unittest.TestCase):
    ...
    def test_pawn_illegal_moves(self):
        self.board = Game(state=game.STARTING_STATE)
    ...
    def test_pawn_take(self):
        self.board = Game(state='rnbqkbnr/pppppppp/P7/8/8/8/1PPPPPPP/RNBQKBNR')
    ...
    def test_pawn_en_passant(self):
        # White En Passant
        self.board = Game(state='rnbqkbnr/pppppp1p/8/5Pp1/8/8/PPPPP1PP/RNBQKBNR w KQkq g6 0 1')
        ...
        # Black En Passant
        self.board = Game(state='rnbqkbnr/pppppp1p/8/8/5Pp1/8/PPPPP1PP/RNBQKBNR b KQkq f3 0 1')
    ...
    def test_rook(self):
        self.board = Game(state='rnbqkbnr/pppppp1p/8/8/P1R1p3/8/1PPP1PPP/1NBQKBNR')
    ...
    def test_bishop(self):
        self.board = Game(state='rnbqkbnr/pppppppp/8/8/2B5/1P2P3/1PPP1PPP/RNBQK1NR')
    ...
    def test_queen(self):
        self.board = Game(state='rnbqkbnr/pppppp1p/B7/8/2Q3p1/1P2P3/1PPP1PPP/RNB1K1NR')
    ...
    def test_knight(self):
        self.board = Game(state='r1bqkbnr/pppp1p1p/B3p3/8/3Q1Np1/1Pn1P3/1PPP1PPP/RNB1K2R')
    ...
    def test_castle_flags(self):
        b = Game(state=game.STARTING_STATE)
    ...
    def test_castling(self):
        ...
        self.board = Game(state='r3kbnr/pp2pppp/5q2/2pp4/1n4b1/2K5/PPPPPPPP/RNBQ1BNR')
        ...
        self.board = Game(state='r3kbnr/pp2pppp/5q2/2pp4/1n6/5b1B/PPPPPP1P/RNBQK1NR')
        with self.assertRaises(IllegalMove):  # Cannot castle if it puts player in check
            self.board._move(position.from_coord('E8'), position.from_coord('C8'))

        self.board = Game(state='r3kbnr/pp2pppp/2N2q2/2pp4/1n6/5b1B/PPPPPP1P/R1BQK1NR')
        with self.assertRaises(IllegalMove):  # Cannot castle if intermediate square is attacked
            self.board._move(position.from_coord('E8'), position.from_coord('C8'))

        # White Queenside castling
        self.board = Game(state='rnbqkbnr/pppppppp/8/8/1P4Q1/B1N1P3/P1PP1PPP/R3KBNR')
        ...
        self.board = Game(state='rnbqk2r/pppp1ppp/4p2n/2b5/8/8/PPPPPPPP/RNBQKBNR')
        king = self.board.squares[position.from_coord('E8').index].piece
        ...
        self.board = Game(state='rnbqkbnr/pppppppp/8/8/8/3BPN2/PPPP1PPP/RNBQK2R')
        king = self.board.squares[position.from_coord('E1').index].piece
