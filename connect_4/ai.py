...
def _alpha_beta(board: Connect4, depth: int, alpha: int, beta: int, is_maximising_player: bool, player: bool):
    ...
    if is_maximising_player:
        ...
        for move in board.legal_moves:
            board.make_move(move)
            score = max(_alpha_beta(board, depth - 1, alpha, beta, not is_maximising_player, player), score)
        ...
        for move in board.legal_moves:
            board.make_move(move)
            score = min(_alpha_beta(board, depth - 1, alpha, beta, not is_maximising_player, player), score)
            beta = min(beta, score)
            board.unmake_move()
            if alpha >= beta:
                return score
